    ...
    function exibirCadastros() {
        ...
        if (mysqli_num_rows($results) > 0) {
            ...
            while ($row = mysqli_fetch_array($results)) {
                ...
                echo                        "<select class='form-select mb-3' name='tipo' autocomplete='off'>";
                echo                            "<option value='Hatch' " . ($row['tipo'] == 'Hatch' ? 'selected' : '') . ">Hatch</option>";
                echo                            "<option value='Sedan' " . ($row['tipo'] == 'Sedan' ? 'selected' : '') . ">Sedan</option>";
                echo                            "<option value='SUV' " . ($row['tipo'] == 'SUV' ? 'selected' : '') . ">SUV</option>";
                echo                        "</select>";
                echo                        "<select class='form-select mb-3' name='status' autocomplete='off'>";
                echo                            "<option selected>Status</option>";
                echo                            "<option value='Disponível' " . ($row['status'] == 'Disponível' ? 'selected' : '') . ">Disponível</option>";
                echo                            "<option value='Alugado' " . ($row['status'] == 'Alugado' ? 'selected' : '') . ">Alugado</option>";
                echo                        "</select>";
                echo                    "</div>";
                echo                "</div>";
